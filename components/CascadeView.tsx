...
import { Layers, Target, CheckSquare, Activity, ChevronDown, ChevronRight, Pencil, Trash2, PieChart, Plus } from 'lucide-react';
...
export const CascadeView: React.FC<CascadeViewProps> = ({
    ...
    const StatusAction = ({ component }: { component: PESComponent }) => (
        <button
            onClick={(e) => { e.preventDefault(); e.stopPropagation(); onShowStatus && onShowStatus(component); }}
            className="p-1.5 text-gray-400 hover:text-brand-purple hover:bg-brand-purple/10 rounded-md transition-colors"
            title="Ver Status de Execução"
        >
    ...
    const renderRow = (
            ...
            return (
                ...
                >
                    ...
                    <div className="flex items-center gap-2 opacity-0 group-hover:opacity-100 transition-opacity bg-white px-3 py-1.5 rounded-xl border border-gray-100 shadow-md transform translate-x-2 group-hover:translate-x-0 z-20">
                        <StatusAction component={component} />
                        {onAddChild && <button onClick={(e) => { e.stopPropagation(); onAddChild(component.id, 'Objetivo'); }} className="p-2 text-gray-400 hover:text-brand-purple hover:bg-brand-purple/10 rounded-lg transition-colors" title="Adicionar Objetivo"><Plus className="w-4 h-4" /></button>}
            ...
            return (
                ...
                >
                    ...
                    <div className="flex items-center gap-1 opacity-0 group-hover/obj:opacity-100 transition-opacity bg-white px-2 py-1 rounded-lg border border-gray-100 shadow-sm z-20">
                        <StatusAction component={component} />
                        {onAddChild && <button onClick={(e) => { e.stopPropagation(); onAddChild(component.id, 'Meta'); }} className="p-1.5 text-gray-400 hover:text-brand-blue hover:bg-brand-blue/10 rounded-md" title="Adicionar Meta"><Plus className="w-3.5 h-3.5" /></button>}
        ...
        if (component.type === 'Meta') {
            ...
            return (
                <div
                    onClick={toggle}
                    className="flex items-start gap-3 py-3 pr-4 pl-4 hover:bg-brand-blue/10 transition-all cursor-pointer group/meta border border-gray-100 bg-white my-2 shadow-sm rounded-lg hover:border-brand-blue hover:shadow-md relative overflow-hidden"
                >
                    ...
                    <div className="flex items-center gap-1 opacity-0 group-hover/meta:opacity-100 transition-opacity bg-white px-2 py-1 rounded-lg border border-gray-100 shadow-sm z-20">
                        <StatusAction component={component} />
                        {onAddChild && <button onClick={(e) => { e.stopPropagation(); onAddChild(component.id, 'Ação'); }} className="p-1.5 text-gray-400 hover:text-sky-500 hover:bg-sky-50 rounded-md" title="Adicionar Ação"><Plus className="w-3.5 h-3.5" /></button>}
